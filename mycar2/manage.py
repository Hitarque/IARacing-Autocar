...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if camera_type == "stereo":
        ...
        #AUTOCAR Traitement d'images

        if cfg.TYPE_TRAITEMENT==1:
            print("----AutoCar: Addinf Traitement parts ")
            from donkeycar.parts.AutoCar_traitementcontour import AC_Imagecontour
            V.add(AC_Imagecontour(),inputs =['cam/image_array'], outputs=['cam/image_array','imageoriginal'],threaded=False)
...
#This web controller will create a web server that is capable
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        if cfg.CONTROLLER_TYPE == "pigpio_rc":    # an RC controllers read by GPIO pins. They typically don't have buttons
            ...
            #Autocar button reader
            if cfg.BUTTON_READER is True:
                from donkeycar.parts.AutoCar_buttonluncher import AC_buttonreader
                V.add(AC_buttonreader(),inputs=['user/mode'],outputs=['user/mode'])
...
#Autocar course reader
    if cfg.ENABLE_YELLOW_DETECTION:
        from donkeycar.parts.AutoCar_coursereader import AC_CourseManage
        V.add(AC_CourseManage(cfg.NOMBRE_DE_TOURS),inputs=['imageoriginal','angle', 'throttle'], outputs=['angle', 'throttle'], threaded=False)
